...
class myPDF extends FPDF {
    ...
    function viewTable($conn) {
        $this->SetFont('courier', '', 10);
        $stmt = $conn->query('select p.cpf, p.nome, c.nome as curso from participante p join participante_tem_curso ptc on p.id=ptc.participante_id join curso c on c.id=ptc.curso_id order by p.nome');
